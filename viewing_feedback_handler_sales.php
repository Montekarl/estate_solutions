<html>
    ...
    <?php 
        ...
        $statement=$pdo->prepare("SELECT * FROM viewings_sales WHERE viewing_id=?");
        $statement->execute([$feedback_id]);
        $fetched_row = $statement->fetch();
        
        /*$sql_query="SELECT * FROM viewings_sales WHERE viewing_id ='$feedback_id'";
        ...
        if(isset($_POST['add_feedback'])){
            $result=$pdo->prepare("UPDATE viewings_sales SET feedback = ? WHERE viewing_id = ?");
            $success=$result->execute([$feedback,$feedback_id]);
            //$sql = "UPDATE viewings_sales SET feedback = '$feedback' WHERE viewing_id ='$feedback_id'";
    ...
    <body>
        ...
        <form action="viewing_feedback_handler_sales.php?feedback_id=<?=$_GET['feedback_id']?>" method="Post">
